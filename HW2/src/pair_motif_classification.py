...
home_dir = "/home/kyle/Documents/CS-674/HW2/hw2_datasets/hw2_datasets"
...
def motif_discovery(data):
    best_so_far = 999999.0
    for i, n in enumerate(data):
        for I, N in enumerate(data):
            if euclidean_dist(N, n) < best_so_far and i!=I:
                best_so_far = euclidean_dist(N,n)
                L1 = i
                L2 = I
    return [L1, L2]

def find_motifs(train):
    """
    Find pair motifs for each class
    """
    train_X = train[:,1:]
    
    # seperate train data by classes
    classes = np.unique(train[:,0])
    class_dict = {}
    for c in classes:
        class_dict[int(c)] = train_X[train[:,0] == c]
                  
    # reference is 10% size of train dataset
    new_train = []
    for key, value in class_dict.items():
        pairs = motif_discovery(value)
        new_train.append(train[pairs[0]])
        new_train.append(train[pairs[1]])
...
train_X, train_y, train = read_train("/dataset"+str(dataset_index)+"/train.txt")
motif_train = find_motifs(train)
motif_train_X = motif_train[:,1:]
motif_train_y = motif_train[:,0]
predicted_y = kNN(k=1, train_X = motif_train_X, train_y=motif_train_y, test_X=test_X, distance_measure='euclidean', window=None)     
